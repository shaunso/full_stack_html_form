...
import { calculateAge, consent } from './utils/form_functions.js'
...
app.post( '/', validators, ( req, res ) => {
  ...
  const usr_alt_mobile_num = `+${country_code_alt_mobile_number ? country_code_alt_mobile_number : '000'}-${ (alt_mobile_number) ? alt_mobile_number : '000000000' }`;
  ...
  const usr_info = [ fname, lname, gender, age, dob, email, id_doc, id_no,usr_mobile_num, usr_alt_mobile_num, address, event_category, emerg_contact_name, usr_emerg_cont_num, usr_event_discover, usr_conset];

  const query = process.env.INSERT_QUERY;

  pool.execute( query, usr_info, ( err, result ) => {
    if (err) throw res.status(500).send(err);
    res.status(200).json({ userData: usr_info} );
...
app.get( '/db', ( req, res ) => {
  pool.execute( process.env.SELECT_QUERY, ( err, result) => {
    if (err) throw res.status(500).send('<h1>Error 580. Something went wrong. Please try again</h1>');
