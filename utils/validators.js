...
[ 
  // first name
  body('fname').trim().escape().notEmpty().withMessage('First name cannot be left blank').bail().matches(/^[A-Za-z ]+$/).withMessage('First name must only contain letters').isLength({min: 3}).withMessage('First name must contain 3 or more letters').bail().isLength({max: 40}).withMessage('First name must contain 40 or less letters'),
  ...
  // gender
  body('gender').trim().escape().notEmpty().withMessage('Gender cannot be left blank').bail().custom( gender => {
  ...
  // mobile number
  body('mobile_number').trim().escape().blacklist(' -').notEmpty().withMessage('Mobile number cannot be left blank').bail().isInt().withMessage('Mobile number must only contain numbers').bail().isLength({min: 7, max:15}).withMessage('Mobile number must contain between 7 and 15 numbers'),
  ...
  // email
  body('email').trim().escape().normalizeEmail().isEmail().withMessage('Enter valid email address').bail().isLength({min:6}).bail().withMessage('Email must contain 6 or more characters').bail().isLength({max:64}).withMessage('Email must contain 64 or less characters'),
  ...
  // race category
  body('event_category').trim().escape().notEmpty().withMessage('Select the race you are entering').bail().custom( race => {
  ...
  body('terms_conds').trim().escape().custom( agree => {
    ...
    throw new Error('Select the checkbox to complete entry')
  }),
